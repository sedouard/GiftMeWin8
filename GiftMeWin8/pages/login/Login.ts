...
(function () {
    ...
    WinJS.UI.Pages.define("/pages/login/Login.html", {
        ...
        ready: function (element, options) {
            var fontSize = "800";
            if ((element.clientWidth == 1440 &&
                element.clientHeight == 1080) ||
                (element.clientWidth == 1024 &&
                element.clientHeight == 768)) {
                fontSize = "800";
            }
            else if ((element.clientWidth == 1920
                && element.clientHeight == 1200) ||
                (element.clientWidth == 1366
                && element.clientHeight == 768) ||
            ...
            else if((element.clientWidth == 1280
                && element.clientHeight == 800)) {
                fontSize = "1700";
            }
...
class GiftMeFacebookAuthenticate {

    // Completion callback
    public static onCompleted: { (suceedded: Boolean, shortTermToken: string): void };

    // Production id
    static s_facebookAppID: string = "175469245988036";

    static callbackFacebookWebAuth(result) {
    ...
    static launchFacebookWebAuth() {
        ...
        try {
            ...
            Windows.Security.Authentication.Web.WebAuthenticationBroker.authenticateAsync(
                ...
                startURI,
                endURI).then(GiftMeFacebookAuthenticate.callbackFacebookWebAuth, GiftMeFacebookAuthenticate.callbackFacebookWebAuthError);
        } catch (err) {
